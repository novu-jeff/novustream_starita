...
<head>
  ...
  <style>
    ...
    * {
    ...
    @font-face {
        font-family: 'EpsonFX';
        src: url('/fonts/Matrix Complex NC.ttf') format('woff');
        font-weight: normal;
        font-style: normal;
    }
...
@php
    ...
    $isPartial = (int) ($cb['isPartial'] ?? 0);
    $partial_payment = (float) ($cb['partial_payment'] ?? 0);
    $isPaid = (int) ($cb['isPaid'] ?? 0);
    ...
    $pesos = intval(floor($paymentAmount));
    $centavos = intval(round(($paymentAmount - $pesos) * 100));
    use App\Helper\NumberHelper;

    $amount_in_words = NumberHelper::convertToWords($paymentAmount);
...
<div id="receipt-overlay" class="printable receipt-sheet overlay-preview" style="display:none;
  ...
  {{-- Date (below OR no.) --}}
  <div style="position:absolute; top:6cm; right:1.5cm; font-size:13px; font-weight:700;">
    {{ \Carbon\Carbon::parse($datePaid)->format('F d, Y') }}
  </div>

  {{-- Agency --}}
  <div style="position:absolute; left: 1.5cm; top:7.0cm; font-size:13px; font-weight:700;">
    SANTA RITA WATER DISTRICT
  </div>

  {{-- Payor --}}
  <div style="position:absolute; left: 1.5cm; top:7.7cm; font-size:12px; text-transform:uppercase; font-weight:700;">
  ...
  <div style="position:absolute; left: 1.5cm; top:9.5cm; font-size:13px; font-weight:700;">
    WB {{ $bill_month }}
  </div>
  <div style="position:absolute; left: 1.5cm; top:10cm; font-size:13px; font-weight:700;">
    @if($isPartial === 1)
        (PARTIAL PAYMENT)
    @endif
    </div>
  @if($isPaid === 1)
    <div style="position:absolute; top:9.2cm; right:1.3cm; width:3.0cm; font-size:14px; text-align:right; font-weight:700;">
  ...
  {{-- Penalty --}}
  @if($applicablePenalty > 0 && $isPaid == 1)
    <div style="position:absolute; top:10.1cm; left: 1.5cm; font-size:14px; font-weight:700;">Penalty</div>
    <div style="position:absolute; top:9.9cm; right:1.3cm; width:3.0cm; font-size:13px; text-align:right; font-weight:700;">₱ {{ number_format($applicablePenalty,2) }}</div>
  @endif

  {{-- Arrears --}}
  @if($arrears > 0 && $isPaid == 1)
    <div style="position:absolute; top:10.9cm; left: 1.5cm; font-size:13px; font-weight:700;">Arrears</div>
    <div style="position:absolute; top:10.7cm; right:1.3cm; width:3.0cm; font-size:13px; text-align:right; font-weight:700;">₱ {{ number_format($arrears,2) }}</div>
  @endif

  {{-- Discount --}}
  @if ($discount > 0)
    <div style="position:absolute; top:13.2cm; left: 1.5cm; font-size:13px; font-weight:700;">Less: Senior Discount</div>
    <div style="position:absolute; top:13.1cm; right:1.4cm; width:3.0cm; font-size:13px; text-align:right; font-weight:700;">₱ {{ number_format($discount,2) }}</div>
  @endif
  {{-- Total --}}
  <!-- <div style="position:absolute; top:12.6cm; font-size:11px; font-weight:700;">TOTAL</div> -->
  @if ($isPaid === 1)
  <div style="position:absolute; top:13.8cm; right:1.4cm; width:3.0cm; font-size:13px; text-align:right; font-weight:700;">₱ {{ number_format($totalAmount, 2) }}</div>
  @endif

  @if ($isPartial === 1)
  <div style="position:absolute; top:13.8cm; right:1.4cm; width:3.0cm; font-size:13px; text-align:right; font-weight:700;">₱ {{ number_format($paymentAmount, 2) }}</div>
  @endif

  {{-- Amount in words --}}
  <div style="position:absolute; left: 1.4cm; top:15.2cm; font-size:13px; text-align:center; font-weight:700;">
    {{ $amount_in_words }}
  </div>

  {{-- Collecting Officer signature --}}
  <div style="position:absolute; bottom:2.4cm; right:1.1cm; text-align:center; width:5.0cm; font-size:11px; ">
